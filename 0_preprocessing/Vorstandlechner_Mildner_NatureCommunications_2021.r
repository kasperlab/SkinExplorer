...
# IO - Setting
#
py$output_dir = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/data_preparation_HCA_v1_0/Vorstandlechner_Mildner_NatureCommunications_2021"
py_run_string("os.makedirs(output_dir, exist_ok=True)")
h5ad_ALL_file = "GSE156326_ALL.h5ad"
metadata_ALL_file = "GSE156326_ALL_metadata.tsv"
h5ad_HC_file = "GSE156326_HC.h5ad"
metadata_HC_file = "GSE156326_HC_metadata.tsv"
######
input_folder = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Vorstandlechner_Mildner_NatureCommunications_2021/GSE156326_RAW"
prefix_unique = sapply(grep("human", list.files(input_folder, pattern = "_matrix.mtx.gz"), value = T), function(x){
...
for(this_prefix in prefix_unique){
  str_fragments = unlist(strsplit(this_prefix, "_", fixed = T))
  this_lib_repo = str_fragments[1]
  this_sample = paste(str_fragments[-1], collapse = "_")
  if(grepl("scar", this_sample)){
    ReadMtx_py("tmp_adata", data_dir = input_folder, prefix = paste0(this_prefix, "_"), features="genes", feature_column_name = c("gene_ids", "feature_name", "feature_types"))
  }else{
    ReadMtx_py("tmp_adata", data_dir = input_folder, prefix = paste0(this_prefix, "_"), features="features", feature_column_name = c("gene_ids", "feature_name"))
  }
...
# Metadata
#
HCA_metadata_list = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Vorstandlechner_2021_HCA_tier 1_metadata.xlsx")
...
rownames(HCA_metadata_list$SP) = HCA_metadata_list$SP[, "sample_id"]
HCA_metadata_list$SP[is.na(HCA_metadata_list$SP)] = "unknown"
#
donor_id_c = HCA_metadata_list$SP[lib_id_repo_c, "donor_id"]
# Sex Identification
...
#
### Study level
py$adata$obs["study_id"] = "Vorstandlechner_Mildner_NatureCommunications_2021"
### Experiment level
py$adata$obs["experiment_id"] = as.character(py$adata$obs[["study_id"]])
py$adata$obs["institute"] = HCA_metadata_list$SP[lib_id_repo_c, "institute"]
py$adata$obs["author_batch_notes"] = HCA_metadata_list$SP[lib_id_repo_c, "author_batch_notes"]
...
py$adata$obs["development_stage_ontology_term_id"] = HCA_metadata_list$SP[lib_id_repo_c, "development_stage_ontology_term_id"]
py$adata$obs["disease"] = dplyr::case_when(
  grepl("skin", sample_id_running_c, fixed = T) ~ "normal",
  grepl("scar", sample_id_running_c, fixed = T) ~ "Scarring",
  .default = sample_id_running_c
)
py$adata$obs["disease_ontology_term_id"] = HCA_metadata_list$SP[lib_id_repo_c, "disease_ontology_term_id"]
py$adata$obs["disease_status"] = dplyr::case_when(
  grepl("skin", sample_id_running_c, fixed = T) ~ "not applicable",
  grepl("scar", sample_id_running_c, fixed = T) ~ "unknown",
  .default = sample_id_running_c
)
...
### Sample level
py$adata$obs["sample_id"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_id"]
py$adata$obs["sample_id_running"] = as.character(py$adata$obs[["donor_id_running"]])
py$adata$obs["sample_source"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_source"]
py$adata$obs["sample_collection_method"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_collection_method"]
py$adata$obs["sampled_site_condition"] = HCA_metadata_list$SP[lib_id_repo_c, "sampled_site_condition"]
py$adata$obs["anatomical_region_level_1"] = dplyr::case_match(
  sample_id_running_c,
  "human_skin_1" ~ "Torso",
  "human_skin_2" ~ "Torso",
  "human_skin_3" ~ "Torso",
  "human_scar_1" ~ "Extremities",
  "human_scar_2" ~ "Extremities",
  "human_scar_3" ~ "Extremities",
  .default = sample_id_running_c
)
py$adata$obs["anatomical_region_level_2"] = dplyr::case_match(
  sample_id_running_c,
  "human_skin_1" ~ "Abdominal",
  "human_skin_2" ~ "Abdominal",
  "human_skin_3" ~ "Abdominal",
  "human_scar_1" ~ "Arm",
  "human_scar_2" ~ "Leg",
  "human_scar_3" ~ "Arm",
  .default = sample_id_running_c
...
py$adata$obs["anatomical_region_level_3"] = dplyr::case_match(
  sample_id_running_c,
  "human_skin_1" ~ "unknown",
  "human_skin_2" ~ "unknown",
  "human_skin_3" ~ "unknown",
  "human_scar_1" ~ "Forearm",
  "human_scar_2" ~ "Calf",
  "human_scar_3" ~ "Axilla",
  .default = sample_id_running_c
)
...
py$adata$obs["tissue_type"] = HCA_metadata_list$SP[lib_id_repo_c, "tissue_type"]
py$adata$obs["tissue_free_text"] = HCA_metadata_list$SP[lib_id_repo_c, "tissue_free_text"]
py$adata$obs["tissue_ontology_term"] = HCA_metadata_list$SP[lib_id_repo_c, "tissue_ontology_term"]
py$adata$obs["tissue_ontology_term_id"] = HCA_metadata_list$SP[lib_id_repo_c, "tissue_ontology_term_id"]
py$adata$obs["skin_tissue"] = "unknown"
py$adata$obs["sample_collection_site"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_collection_site"]
py$adata$obs["sample_cultured"] = "No"
### Sequencing level
py$adata$obs["sequencing_type"] = "GEX"
py$adata$obs["sample_collection_year"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_collection_year"]
py$adata$obs["sample_collection_relative_time_point"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_collection_relative_time_point"]
py$adata$obs["library_id"] = HCA_metadata_list$SP[lib_id_repo_c, "library_id"]
py$adata$obs["library_id_repository"] = lib_id_repo_c
py$adata$obs["library_preparation_batch"] = HCA_metadata_list$SP[lib_id_repo_c, "library_preparation_batch"]
py$adata$obs["library_sequencing_run"] = HCA_metadata_list$SP[lib_id_repo_c, "library_sequencing_run"]
py$adata$obs["sample_preservation_method"] = HCA_metadata_list$SP[lib_id_repo_c, "sample_preservation_method"]
py$adata$obs["protocol_url"] = "unknown"
py$adata$obs["dissociation_protocol"] = HCA_metadata_list$SP[lib_id_repo_c, "dissociation_protocol"]
py$adata$obs["cell_enrichment"] = HCA_metadata_list$SP[lib_id_repo_c, "cell_enrichment"]
py$adata$obs["cell_viability_percentage"] = HCA_metadata_list$SP[lib_id_repo_c, "cell_viability_percentage"]
py$adata$obs["cell_number_loaded"] = HCA_metadata_list$SP[lib_id_repo_c, "cell_number_loaded"]
py$adata$obs["suspension_type"] = HCA_metadata_list$SP[lib_id_repo_c, "suspension_type"]
...
py$adata$obs["sequencing_platform"] = "Illumina HiSeq 3000"
### Preprocessing level
...
### Previous Barcode
this_dataset_individual = "Vorstandlechner 2021"
this_dataset_integrated = "Vorstandlechner_Mildner_NatureCommunications_2021"
...
individual_barcode_all = individual_barcode_used[paste0(py$adata$obs[["study_id"]], "___", stringi::stri_replace_all_fixed(str = py$adata$obs_names$values,
                                                                                                                           pattern = c("human_skin_1", "human_skin_2", "human_skin_3"),
                                                                                                                           replacement = c("Skin1", "Skin2", "Skin3"),
                                                                                                                           vectorize_all = FALSE))]
