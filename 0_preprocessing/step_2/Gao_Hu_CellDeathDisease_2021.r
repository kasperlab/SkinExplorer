...
# IO - Setting
#
py$output_dir = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/data_preparation_HCA_v1_0/Gao_Hu_CellDeathDisease_2021"
py_run_string("os.makedirs(output_dir, exist_ok=True)")
h5ad_ALL_file = "GSE162183_ALL.h5ad"
metadata_ALL_file = "GSE162183_ALL_metadata.tsv"
h5ad_HC_file = "GSE162183_HC.h5ad"
metadata_HC_file = "GSE162183_HC_metadata.tsv"
######
...
py_run_string("adata_raw = sc.read_loom(loom_path)")
py_run_string("adata_raw.obsm = {'Original_UMAP': np.column_stack([adata_raw.obs['_UMAP1'].astype(np.float64), adata_raw.obs['_UMAP2'].astype(np.float64)]), 'Original_TSNE': np.column_stack([adata_raw.obs['_tSNE1'].astype(np.float64), adata_raw.obs['_tSNE2'].astype(np.float64)])}")
donor_id_running_c = as.character(py$adata_raw$obs[["Patient"]])
py_run_string("del adata_raw.obs")
...
# Metadata
#
HCA_metadata_list = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Gao_2021_HCA_tier 1_metadata.xlsx")
HCA_metadata_list_original = HCA_metadata_list # Keep the original for checking later
rownames(HCA_metadata_list$DN) = c("Ctrl1", "Ctrl2", "Ctrl3", "Psor1", "Psor2", "Psor3")
HCA_metadata_list$DN[is.na(HCA_metadata_list$DN)] = "unknown"
rownames(HCA_metadata_list$SP) = c("Ctrl1", "Ctrl2", "Ctrl3", "Psor1", "Psor2", "Psor3")
...
#
### Study level
py$adata$obs["study_id"] = "Gao_Hu_CellDeathDisease_2021"
### Experiment level
...
py$adata$obs["disease"] = dplyr::case_when(
  grepl("Ctrl", donor_id_running_c, fixed = T) ~ "normal",
  grepl("Psor", donor_id_running_c, fixed = T) ~ "psoriasis",
  .default = donor_id_running_c
)
...
py$adata$obs["disease_status"] = dplyr::case_when(
  grepl("Ctrl", donor_id_running_c, fixed = T) ~ "not applicable",
  grepl("Psor", donor_id_running_c, fixed = T) ~ "unknown",
  .default = donor_id_running_c
)
...
### Previous Barcode
this_dataset_individual = "Gao 2021"
this_dataset_integrated = "Gao_Hu_CellDeathDisease_2021"
...
py_run_string("sc.pl.embedding(adata, 'Original_UMAP', color=['donor_id_running'], size=8, frameon=False, ncols=1, save='_donor_id_running.pdf')")
py_run_string("sc.pl.embedding(adata, 'Original_TSNE', color=['donor_id_running'], size=8, frameon=False, ncols=1, save='_donor_id_running.pdf')")
#
