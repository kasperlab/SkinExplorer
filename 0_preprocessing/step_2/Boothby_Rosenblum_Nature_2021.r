...
# IO - Setting
#
py$output_dir = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/data_preparation_HCA_v1_0/Boothby_Rosenblum_Nature_2021"
py_run_string("os.makedirs(output_dir, exist_ok=True)")
h5ad_ALL_file = "GSE183031_ALL.h5ad"
metadata_ALL_file = "GSE183031_ALL_metadata.tsv"
h5ad_HC_file = "GSE183031_HC.h5ad"
metadata_HC_file = "GSE183031_HC_metadata.tsv"
######
py$h5_path_HC01 = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Boothby_Rosenblum_Nature_2021/GSE183031_RAW/GSM5549903_HC01_filtered_feature_bc_matrix.h5"
py$h5_path_HC02 = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Boothby_Rosenblum_Nature_2021/GSE183031_RAW/GSM5549904_HC02_filtered_feature_bc_matrix.h5"
py$h5_path_HC03 = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Boothby_Rosenblum_Nature_2021/GSE183031_RAW/GSM5549905_HC03_filtered_feature_bc_matrix.h5"
py$h5_path_EF01 = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Boothby_Rosenblum_Nature_2021/GSE183031_RAW/GSM5549906_EF01_filtered_feature_bc_matrix.h5"
######
py_run_string("GEX_dict = {}")
for(ii in c("HC01", "HC02", "HC03", "EF01")){
  py_run_string(paste0("GEX_dict['", ii, "'] = sc.read_10x_h5(h5_path_", ii, ")"))
  py_run_string(paste0("GEX_dict['", ii, "'].obs_names = '", ii, "_' + GEX_dict['", ii, "'].obs_names.astype(str).values"))
  py_run_string(paste0("GEX_dict['", ii, "'].var['feature_name'] = GEX_dict['", ii, "'].var_names.values.copy()"))
  py_run_string(paste0("GEX_dict['", ii, "'].var_names = GEX_dict['", ii, "'].var['gene_ids'].copy()"))
  py_run_string(paste0("del h5_path_", ii))
}
#
py_run_string("adata = ad.concat(GEX_dict, axis=0, join='outer')")
py_run_string("SameVar = AssertSameDictAdataVar(GEX_dict)")
if(py$SameVar){
  py_run_string("adata.var = GEX_dict['HC01'].var.copy()")
}
...
# Metadata
#
donor_id_running_c = as.character(sapply(py$adata$obs_names$values, function(x){
  return(unlist(strsplit(x, "_", fixed = T))[1])
}))
#
HCA_metadata_list = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Boothby_2021_HCA_tier 1_metadata.xlsx")
HCA_metadata_list_original = HCA_metadata_list # Keep the original for checking later
rownames(HCA_metadata_list$DN) = c("HC01", "HC02", "HC03", "EF01")
HCA_metadata_list$DN[is.na(HCA_metadata_list$DN)] = "unknown"
rownames(HCA_metadata_list$SP) = c("HC01", "HC02", "HC03", "EF01")
...
#
### Study level
py$adata$obs["study_id"] = "Boothby_Rosenblum_Nature_2021"
### Experiment level
...
py$adata$obs["disease"] = dplyr::case_match(
  donor_id_running_c,
  "HC01" ~ "Mohs Surgery Necessary",
  "HC02" ~ "Mohs Surgery Necessary",
  "HC03" ~ "Mohs Surgery Necessary",
  "EF01" ~ "eosinophilic fasciitis",
  .default = donor_id_running_c
)
py$adata$obs["disease_ontology_term_id"] = dplyr::case_match(
  ...
  "EF01" ~ "MONDO:0009175",
  .default = donor_id_running_c
)# HCA_metadata_list$SP[donor_id_running_c, "disease_ontology_term_id"]
py$adata$obs["disease_status"] = "unknown"
...
py$adata$obs["sample_collection_method"] = dplyr::case_when(
  grepl("HC", donor_id_running_c, fixed = T) ~ "surgical resection",
  grepl("EF", donor_id_running_c, fixed = T) ~ "biopsy",
  .default = donor_id_running_c
)
py$adata$obs["sampled_site_condition"] = dplyr::case_when(
  grepl("HC", donor_id_running_c, fixed = T) ~ "healthy",
  grepl("EF", donor_id_running_c, fixed = T) ~ "diseased",
  .default = donor_id_running_c
)
...
### Previous Barcode
this_dataset_individual = "Boothby 2021"
this_dataset_integrated = "Boothby_Rosenblum_Nature_2021"
