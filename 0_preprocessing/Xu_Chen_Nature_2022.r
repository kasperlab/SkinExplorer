...
# IO - Setting
#
py$output_dir = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/data_preparation_HCA_v1_0/Xu_Chen_Nature_2022"
py_run_string("os.makedirs(output_dir, exist_ok=True)")
h5ad_ALL_file = "OMIX691_ALL.h5ad"
metadata_ALL_file = "OMIX691_ALL_metadata.tsv"
h5ad_HC_file = "OMIX691_HC.h5ad"
metadata_HC_file = "OMIX691_HC_metadata.tsv"
######
input_folder = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Xu_Chen_Nature_2022/OMIX691"
donor_id_running_c = c()
py_run_string("GEX_dict = {}")
for(this_mtx_path in list.files(input_folder, recursive = T, pattern = "matrix.mtx")){
  str_fragments = unlist(strsplit(this_mtx_path, "/", fixed = T))
  this_donor_id_running = str_fragments[2]
  this_folder_path = paste(str_fragments[-length(str_fragments)], collapse = "/")
  ReadMtx_py("tmp_adata", data_dir = paste0(input_folder, "/", this_folder_path), prefix = "", features="genes", feature_column_name = c("gene_ids", "feature_name"), is_compressed_file=F)
  py_run_string(paste0("tmp_adata.obs_names = '", this_donor_id_running, "_' + tmp_adata.obs_names.astype(str).values"))
  py_run_string(paste0("GEX_dict['", this_donor_id_running, "'] = tmp_adata"))
  donor_id_running_c = c(donor_id_running_c, rep(this_donor_id_running, py$tmp_adata$n_obs))
  print(this_donor_id_running)
}
#
...
# Metadata
#
HCA_metadata_list = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Xu_2021_HCA_tier 1_metadata.xlsx")
HCA_metadata_list_original = HCA_metadata_list # Keep the original for checking later
HCA_metadata_list_Hao = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Xu_2021_HCA_tier 1_metadata_Hao.xlsx")
HCA_metadata_list$DN = HCA_metadata_list_Hao$DN
HCA_metadata_list$SP = HCA_metadata_list_Hao$SP
rownames(HCA_metadata_list$DN) = c("H01", "H02", "H03", "H04", "H05", "P01", "P02", "P03", "P04", "P05", "P06", "P07", "P08", "P09", "P10")
HCA_metadata_list$DN[is.na(HCA_metadata_list$DN)] = "unknown"
rownames(HCA_metadata_list$SP) = c("H01", "H02", "H03", "H04", "H05", "P01", "P02", "P03", "P04", "P05", "P06", "P07", "P08", "P09", "P10")
...
#
### Study level
py$adata$obs["study_id"] = "Xu_Chen_Nature_2022"
### Experiment level
...
py$adata$obs["disease"] = dplyr::case_when(
  grepl("H", donor_id_running_c, fixed = T) ~ "normal",
  grepl("P", donor_id_running_c, fixed = T) ~ "vitiligo",
  .default = donor_id_running_c
)
...
py$adata$obs["skin_tissue"] = "Epidermis + Dermis"
...
py$adata$obs["sample_preservation_method"] = HCA_metadata_list$SP[donor_id_running_c, "sample_preservation_method"]
...
py$adata$obs["alignment_software"] = "cell ranger 1.3.1"
...
### Previous Barcode
this_dataset_individual = "Xu 2022"
this_dataset_integrated = "Xu_Chen_Nature_2022"
