...
# IO - Setting
#
py$output_dir = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/data_preparation_HCA_v1_0/Ji_Khavari_Cell_2020"
py_run_string("os.makedirs(output_dir, exist_ok=True)")
h5ad_ALL_file = "GSE144236_ALL.h5ad"
metadata_ALL_file = "GSE144236_ALL_metadata.tsv"
h5ad_ADJACENT_file = "GSE144236_ADJACENT.h5ad"
metadata_ADJACENT_file = "GSE144236_ADJACENT_metadata.tsv"
######
txt_path = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Ji_Khavari_Cell_2020/GSE144236_cSCC_counts.txt.gz"
this_df = as.data.frame(fread(txt_path))
rownames(this_df) = this_df[, 1]
this_obj = CreateSeuratObject(as.matrix(this_df[-c(1, 2), -1]), min.cells = 1)
rm(this_df)
######
...
sample_id_running_c = sapply(py$adata$obs_names$values, function(x){
  ...
  return(paste(str_fragments[c(1, 2)], collapse = "_"))
})
donor_id_running_c = sapply(py$adata$obs_names$values, function(x){
  str_fragments = unlist(strsplit(x, "_"))
  return(paste(str_fragments[1], collapse = "_"))
})
#
GSE144236_metadata_path = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Ji_Khavari_Cell_2020/GSE144236_patient_metadata_new.txt.gz"
metadata_raw = as.matrix(read.table(GSE144236_metadata_path, header = T, row.names = 1, sep = "\t"))
metadata = metadata_raw[py$adata$obs_names$values, ]
author_cell_type_1 = metadata[, "level1_celltype"]
author_cell_type_2 = metadata[, "level2_celltype"]
author_cell_type_3 = metadata[, "level3_celltype"]
rm(metadata_raw)
#
HCA_metadata_list = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Ji_2020_HCA_tier 1_metadata.xlsx")
HCA_metadata_list_original = HCA_metadata_list # Keep the original for checking later
HCA_metadata_list_Hao = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Ji_2020_HCA_tier 1_metadata_Hao.xlsx")
...
#
### Study level
py$adata$obs["study_id"] = "Ji_Khavari_Cell_2020"
### Experiment level
py_run_string("adata.obs['experiment_id'] = adata.obs['study_id'].astype(str) + '_' + 'GSE144236'")
...
py$adata$obs["disease"] = "squamous cell carcinoma"
...
py$adata$obs["disease_status"] = dplyr::case_match(
  donor_id_running_c,
  "P1" ~ "Well Differentiated - Untreated",
  "P2" ~ "Well Differentiated - Untreated",
  "P3" ~ "Well Differentiated - Untreated",
  "P4" ~ "Well Differentiated - Untreated",
  "P5" ~ "Well Differentiated - Untreated",
  "P6" ~ "Well Differentiated - Untreated",
  "P7" ~ "Moderate Differentiated - Untreated",
  "P8" ~ "Moderate Differentiated - Untreated",
  "P9" ~ "Moderate Differentiated - Untreated",
  "P10" ~ "Moderate Differentiated - Recurrent",
  .default = donor_id_running_c
)
...
py$adata$obs["cell_enrichment"] = dplyr::case_match(
  donor_id_running_c,
  "P1" ~ "CD45-",
  "P2" ~ "unknown",
  "P3" ~ "unknown",
...
### Previous Barcode
this_dataset_individual = "Ji 2020"
this_dataset_integrated = "Ji_Khavari_Cell_2020"
...
py_run_string("adata_ADJACENT = adata[adata.obs['sampled_site_condition'] == 'adjacent']")
py_run_string(paste0("adata_ADJACENT.write(output_dir + '/", h5ad_ADJACENT_file, "')"))
py_run_string(paste0("adata_ADJACENT.obs.to_csv(output_dir + '/", metadata_ADJACENT_file, "', sep='\t', index=True)"))
