...
# IO - Setting
#
py$output_dir = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/data_preparation_HCA_v1_0/Kim_Nagao_NatureMedicine_2020"
py_run_string("os.makedirs(output_dir, exist_ok=True)")
h5ad_ALL_file = "GSE132802_ALL.h5ad"
metadata_ALL_file = "GSE132802_ALL_metadata.tsv"
h5ad_HC_file = "GSE132802_HC.h5ad"
metadata_HC_file = "GSE132802_HC_metadata.tsv"
######
input_folder = "D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/Kim_Nagao_NatureMedicine_2020/GSE132802_RAW"
sample_prefixes = c("GSM3892569_Skin_Dress1", "GSM3892577_SKIN_HV1_F1", "GSM3892578_SKIN_HV1_F2", "GSM3892579_SKIN_HV2", "GSM3892580_SKIN_HV3", "GSM3892581_SKIN_HV4", "GSM3892582_SKIN_HV5")
lib_id = c()
sample_id_running_c = c()
py_run_string("GEX_dict = {}")
for(ii in sample_prefixes){
  str_fragments = unlist(strsplit(ii, "_", fixed = T))
  this_sample = paste(str_fragments[-1], collapse = "_")
  this_lib = str_fragments[1]
  py_run_string(paste0("GEX_dict['", this_sample, "'] = sc.read_10x_h5('", input_folder, "/", ii, "_filtered_gene_bc_matrices_h5.h5')"))
  py_run_string(paste0("GEX_dict['", this_sample, "'].obs_names = '", this_sample, "_' + GEX_dict['", this_sample, "'].obs_names.astype(str).values"))
  py_run_string(paste0("GEX_dict['", this_sample, "'].var['feature_name'] = GEX_dict['", this_sample, "'].var_names.values.copy()"))
  py_run_string(paste0("GEX_dict['", this_sample, "'].var_names = GEX_dict['", this_sample, "'].var['gene_ids'].copy()"))
  lib_id = c(lib_id, rep(this_lib, eval(parse(text = paste0("py$GEX_dict$'", this_sample, "'$n_obs")))))
...
if(py$SameVar){
  py_run_string("adata.var = GEX_dict['Skin_Dress1'].var.copy()")
}
...
# Metadata
#
lib_id_c = as.character(lib_id)
#
HCA_metadata_list = load_HCA_metadata_xlsx("D:/Study/KI/Projects/###/MetaStudies/Data/Skin/Human/HCA_metadata/Kim_2020_HCA_tier 1_metadata.xlsx")
...
#
### Study level
py$adata$obs["study_id"] = "Kim_Nagao_NatureMedicine_2020"
### Experiment level
...
py$adata$obs["age_range"] = HCA_metadata_list_LIB[lib_id_c, "age_range"]
...
py$adata$obs["disease"] = dplyr::case_match(
  donor_id_c,
  "D_DRESS_patient" ~ "drug rash with eosinophilia and systemic symptoms",
  "D_HV1" ~ "normal",
  "D_HV2" ~ "normal",
  "D_HV3" ~ "normal",
  "D_HV4" ~ "normal",
...
py$adata$obs["disease_status"] = dplyr::case_match(
  donor_id_c,
  "D_DRESS_patient" ~ "unknown",
  "D_HV1" ~ "not applicable",
  "D_HV2" ~ "not applicable",
  "D_HV3" ~ "not applicable",
  "D_HV4" ~ "not applicable",
  "D_HV5" ~ "not applicable",
  .default = donor_id_c
)
...
py$adata$obs["anatomical_region_level_3"] = "Flank"
...
py$adata$obs["cell_number_loaded"] = HCA_metadata_list_LIB[lib_id_c, "cell_number_loaded"]
py$adata$obs["suspension_type"] = "cell"
py$adata$obs["assay_ontology_term"] = dplyr::case_match(
  donor_id_c,
  "D_DRESS_patient" ~ "10x 5' v2",
  "D_HV1" ~ "10x 3' v2",
  "D_HV2" ~ "10x 3' v2",
  "D_HV3" ~ "10x 3' v2",
  "D_HV4" ~ "10x 5' v2",
...
py$adata$obs["sequenced_fragment"] = dplyr::case_match(
  donor_id_c,
  "D_DRESS_patient" ~ "5 prime tag",
  "D_HV1" ~ "3 prime tag",
  "D_HV2" ~ "3 prime tag",
  "D_HV3" ~ "3 prime tag",
  "D_HV4" ~ "5 prime tag",
...
### Previous Barcode
this_dataset_individual = "Kim 2020"
this_dataset_integrated = "Kim_Nagao_NatureMedicine_2020"
...
individual_barcode_all = individual_barcode_used[stringi::stri_replace_all_fixed(str = py$adata$obs_names$values,
                                                                                 pattern = c("SKIN_HV1_F1", "SKIN_HV1_F2"),
                                                                                 replacement = c("SKIN_HV1_duplicate1", "SKIN_HV1_duplicate2"),
                                                                                 vectorize_all = FALSE)]
